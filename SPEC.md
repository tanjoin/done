...
- Drive から読み込んだ revision・version・ファイルID・未同期変更の有無をローカルに同期状態として保持する
...
- Drive の単調増加する version を保存前の変更検出に使用し、保存後は自分が保存した revision がDriveに残っていることを再読込で検証する
...
- 自動マージまたは選択解消の後は、最新のDrive versionを基準に再同期する。保存直前または保存後に別変更を検出した場合は、最大2回まで再読込・再マージ・再保存する
