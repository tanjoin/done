...
- remindMinutesBefore が明示設定されているタスクは、時間外フィルタの対象外として時間内同等に扱う
...
- 再ログイン直後は、Google Drive に既存タスクデータがある場合、ローカルとDriveのタスクをID単位でマージして両方の変更を保持する
- マージ時は片方にのみあるタスクを残し、同一IDのhistoryは日付キー単位で併合する。同じ日付が競合する場合は再ログイン前にローカルで行った操作を優先する
- マージ完了後は統合結果をGoogle Driveへ保存する
...
- Google のログインセッションまたは認可状態が失効した場合の再ログイン導線は設定画面に限定する
