...
- 再ログイン直後は、Google Drive に既存タスクデータがある場合、ローカルとDriveのタスクをID単位でマージしてローカルへ反映する。ログインまたは読み込みだけではDriveへ保存しない
- マージ時は片方にのみあるタスクを残し、同一IDのhistoryは日付キー単位で併合する。同じ日付が競合する場合は再ログイン前にローカルで行った操作を優先する
- マージ結果は、その後の明示的なタスク更新またはインポート時に通常の同期ルールでGoogle Driveへ保存する
- Googleログイン成功後は、Driveデータのマージ、TODOカレンダーの再取得、一覧の再描画を順に実行する
- 通常同期では、更新元（ローカル）の最終更新日時が欠落している場合は上書きを停止する
- 保存先（Google Drive）の最終更新日時が欠落している場合は、ローカルデータで保存先を上書きしてタイムスタンプを補完する
