import{D as e,E as t,M as n,N as r,O as i,P as a,T as o,_ as s,a as c,b as l,d as u,f as d,g as f,i as p,k as m,l as h,m as g,n as _,o as v,p as y,r as b,s as x,t as S,u as C,v as w,w as T,y as E}from"./google-auth-alert-C8HMTw7U.js";import{a as D,c as O,d as k,i as A,l as j,n as M,o as N,r as P,t as F,u as I}from"./task-repository-D18RXe2h.js";var L,R=a((()=>{o(),E(),v(),O(),d(),L=class{static render(){return`
    ...
    `}static setup(e,t){let n=e.querySelector(`#calendarSection`),r=e.querySelector(`#googleClientIdInput`),i=e.querySelector(`#calendarSettingForm`),a=e.querySelector(`#todoCalendarSelect`),o=e.querySelector(`#todoCalendarSelect2`),s=e.querySelector(`#doneCalendarSelect`),d=e.querySelector(`#loadCalendarListBtn`),p=e.querySelector(`#doneCalendarManualInput`),m=e.querySelector(`#googleLoginBtn`),h=e.querySelector(`#googleLoginStatus`),_=e.querySelector(`#googleSaveStatus`),v=e.querySelector(`#googleDriveLinkStatus`),b=!1;if(!T.supportsLocalStorage()){n&&(n.style.display=`none`);return}if(!r||!i||!a||!o||!s||!p||!m||!h||!v)return;I().then(e=>{r.value=e.clientId;let t=e.todoCalendarIds.length?e.todoCalendarIds:e.todoCalendarId?[e.todoCalendarId]:[],n=[`<option value="">選択してください</option>`];a.innerHTML=n.join(``),o.innerHTML=n.join(``),t[0]&&(a.innerHTML=`<option value="${t[0]}">${t[0]}</option>`),t[1]&&(o.innerHTML=`<option value="${t[1]}">${t[1]}</option>`),a.value=t[0]||``,o.value=t[1]||``,p.value=e.doneCalendarId}),p.value=T.calendarTargetId;let S=()=>{if(!h)return;let e=w();h.textContent=e?`ログイン済み`:`未ログイン`,m.textContent=e?`Googleからログアウト`:`Googleにログイン`},C=async()=>{if(v.style.display=`block`,!w()){v.textContent=`Google Drive 保存先リンクはログイン後に表示されます。`;return}try{let e=await c();if(!e){v.textContent=`Google Drive 保存先ファイルはまだ作成されていません。`;return}v.innerHTML=`Google Drive 保存先: <a href="${e}" target="_blank" rel="noopener noreferrer">バックアップファイルを開く</a>`}catch{v.textContent=`Google Drive 保存先リンクの取得に失敗しました。`}};S(),C(),m.addEventListener(`click`,async()=>{if(w()){g(),S(),u.notifyGoogleSessionStateChanged(),await C(),t.hide();return}let e=r.value.trim();if(!e){t.show(`先に OAuth 2.0 Client ID を入力してからログインしてください。`);return}try{let n=await I();await k({clientId:e,todoCalendarIds:n.todoCalendarIds,doneCalendarId:p.value.trim()||n.doneCalendarId}),await f(y,!0),S(),u.notifyGoogleSessionStateChanged();let r=await x();r!==null&&!T.taskSyncDirty&&(T.tasks=r.tasks,r.updatedAt&&(T.tasksLastUpdatedAt=r.updatedAt),T.taskSyncState={baseRevision:r.revision,baseDriveVersion:r.version,fileId:r.fileId,dirty:!1,baseTasks:r.tasks}),u.notifyGoogleLoginSucceeded(),await C(),t.hide()}catch(e){S(),u.notifyGoogleSessionStateChanged(),await C();let n=e instanceof Error?e.message:`Googleログインに失敗しました。`;t.show(n)}}),d?.addEventListener(`click`,async()=>{if(!r.value.trim()){t.show(`OAuth 2.0 Client ID を入力してからカレンダー一覧を取得してください。`);return}try{let e=await I();await k({clientId:r.value,todoCalendarIds:e.todoCalendarIds,doneCalendarId:e.doneCalendarId});let n=await j(),i=[`<option value="">選択してください</option>`].concat(n.map(e=>`<option value="${e.id}">${e.summary} (${e.id})</option>`)).join(``);a.innerHTML=i,o.innerHTML=i,s.innerHTML=i;let c=e.todoCalendarIds.length?e.todoCalendarIds:e.todoCalendarId?[e.todoCalendarId]:[];a.value=c[0]||``,o.value=c[1]||``,s.value=e.doneCalendarId,t.hide()}catch(e){let n=l(e),r=e instanceof Error?e.message:`一覧取得に失敗しました。`;if(n){t.show(r);return}alert(r)}}),s.addEventListener(`change`,()=>{b=!0}),i.addEventListener(`submit`,async e=>{e.preventDefault();try{let e=p.value.trim()||(b?s.value.trim():``),t=[a.value,o.value].map(e=>e.trim()).filter(Boolean);await k({clientId:r.value,todoCalendarIds:t,doneCalendarId:e}),p.value=e,_&&(_.style.display=`inline`,setTimeout(()=>{_.style.display=`none`},2500)),S(),await C()}catch{alert(`設定保存に失敗しました。`)}})}}})),z,B=a((()=>{m(),v(),z=class e{static excludeGoogleTodoTasks(e){return e.filter(e=>e.sourceType!==`google-todo`)}static render(){return`
      <div class="data-box">
