import{E as e,T as t,_ as n,a as r,b as i,c as a,g as o,h as s,i as c,l,m as u,n as d,r as f,s as p,t as m,v as h,w as g,x as _,y as v}from"./google-drive-service-BR5EmNtU.js";import{a as y,c as b,d as x,i as S,l as C,n as w,o as T,r as E,t as D,u as O}from"./task-repository-DR2lIpMB.js";var k,A,j=e((()=>{s(),l(),d(),b(),k=[`https://www.googleapis.com/auth/calendar`,`https://www.googleapis.com/auth/calendar.events`,`https://www.googleapis.com/auth/drive.file`],A=class{static render(){return`
    ...
    `}static setup(e){let t=e.querySelector(`#calendarSection`),n=e.querySelector(`#googleClientIdInput`),i=e.querySelector(`#calendarSettingForm`),o=e.querySelector(`#todoCalendarSelect`),s=e.querySelector(`#doneCalendarSelect`),l=e.querySelector(`#loadCalendarListBtn`),d=e.querySelector(`#doneCalendarManualInput`),h=e.querySelector(`#googleLoginBtn`),g=e.querySelector(`#googleLoginStatus`),_=e.querySelector(`#googleDriveSyncToggle`),v=e.querySelector(`#googleSaveStatus`),y=e.querySelector(`#googleDriveLinkStatus`),b=!1;if(!u.supportsLocalStorage()){t&&(t.style.display=`none`);return}if(!n||!i||!o||!s||!d||!h||!g||!y||!_)return;O().then(e=>{n.value=e.clientId,e.todoCalendarId&&(o.innerHTML=`<option value="${e.todoCalendarId}">${e.todoCalendarId}</option>`),d.value=e.doneCalendarId}),_.checked=u.googleDriveSyncEnabled,d.value=u.calendarTargetId;let S=()=>{if(!g)return;let e=a();g.textContent=e?`ログイン済み`:`未ログイン`,h.textContent=e?`Googleからログアウト`:`Googleにログイン`},w=async()=>{if(!_.checked){y.style.display=`none`,y.innerHTML=``;return}if(y.style.display=`block`,!a()){y.textContent=`Google Drive 保存先リンクはログイン後に表示されます。`;return}try{let e=await m();if(!e){y.textContent=`Google Drive 保存先ファイルはまだ作成されていません。`;return}y.innerHTML=`Google Drive 保存先: <a href="${e}" target="_blank" rel="noopener noreferrer">バックアップファイルを開く</a>`}catch{y.textContent=`Google Drive 保存先リンクの取得に失敗しました。`}};S(),w(),h.addEventListener(`click`,async()=>{if(a()){r(),S(),await w();return}let e=n.value.trim();if(!e){alert(`先に OAuth 2.0 Client ID を入力してください。`);return}try{await p(k,!0);let t=await O();if(await x({clientId:e,todoCalendarId:t.todoCalendarId,doneCalendarId:d.value.trim()||t.doneCalendarId}),S(),_.checked){let e=await f();if(e!==null){let t=u.tasksLastUpdatedAt;e.hasTimestamp&&(!t||t<=e.updatedAt)&&(u.tasks=e.tasks,u.tasksLastUpdatedAt=e.updatedAt)}else await c(u.tasks)}await w()}catch(e){S(),await w();let t=e instanceof Error?e.message:`Googleログインに失敗しました。`;alert(t)}}),l?.addEventListener(`click`,async()=>{if(!n.value.trim()){alert(`OAuth 2.0 Client ID を入力してください。`);return}try{let e=await O();await x({clientId:n.value,todoCalendarId:e.todoCalendarId,doneCalendarId:e.doneCalendarId});let t=await C(),r=[`<option value="">選択してください</option>`].concat(t.map(e=>`<option value="${e.id}">${e.summary} (${e.id})</option>`)).join(``);o.innerHTML=r,s.innerHTML=r,o.value=e.todoCalendarId,s.value=e.doneCalendarId}catch(e){let t=e instanceof Error?e.message:`一覧取得に失敗しました。`;alert(t)}}),s.addEventListener(`change`,()=>{b=!0}),_.addEventListener(`change`,()=>{w()}),i.addEventListener(`submit`,async e=>{e.preventDefault();try{let e=d.value.trim()||(b?s.value.trim():``);await x({clientId:n.value,todoCalendarId:o.value,doneCalendarId:e}),d.value=e,u.googleDriveSyncEnabled=_.checked,v&&(v.style.display=`inline`,setTimeout(()=>{v.style.display=`none`},2500)),S(),await w()}catch{alert(`設定保存に失敗しました。`)}})}}})),M,N=e((()=>{_(),M=class e{static excludeGoogleTodoTasks(e){return e.filter(e=>e.sourceType!==`google-todo`)}static render(){return`
      <div class="data-box">
    ...
    `}static setup(e){let t=e.querySelector(`#overdueReferenceDateInput`),n=e.querySelector(`#overdueReferenceDateForm`),r=e.querySelector(`#displaySaveStatus`);!t||!n||(t.value=u.overdueReferenceDate,n.addEventListener(`submit`,e=>{e.preventDefault(),t.value&&(u.overdueReferenceDate=t.value,r&&(r.style.display=`inline`,setTimeout(()=>{r.style.display=`none`},2500)))}))}}})),I,L=e((()=>{s(),S(),T(),I=class e{static render(){return`
      ${e.renderSound()}
      ...
      <div class="data-box" id="soundSection">
        ...
        <div class="setting-row">
          ...
          <select id="notificationSoundSelect" class="setting-input">
            ${y.options.map(e=>`<option value="${e.value}">${e.label}</option>`).join(``)}
          </select>
        </div>
    ...
    `}async setup(){this._taskRepository.hydrateFromLocal(),R.setup(this),A.setup(this),P.setup(this),I.setup(this),M.setup(this,this._taskRepository),document.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof Element))return;let n=t.closest(`a[href]`);n&&n.getAttribute(`href`)===`index.html`&&D.markNextIndexNavigationFromSettings()}),this._taskRepository.refreshFromCloudIfNeeded()}};customElements.get(e.NAME)||customElements.define(e.NAME,e),document.addEventListener(`DOMContentLoaded`,async()=>{let t=document.querySelector(`.container`);if(!t)return;let n=document.createElement(o.NAME);n.active=`settings`,t.appendChild(n);let r=document.createElement(e.NAME);t.appendChild(r);let i=document.createElement(h.NAME);t.appendChild(i)})}))();