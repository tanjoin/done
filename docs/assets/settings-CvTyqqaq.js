import{C as e,D as t,E as n,O as r,S as i,_ as a,a as o,b as s,d as c,g as l,i as u,l as d,n as f,o as p,r as m,s as h,t as g,u as _,v,x as y,y as b}from"./session-manager-BgRpcS4h.js";import{a as x,c as S,d as C,i as w,l as T,n as E,o as D,r as O,t as k,u as A}from"./task-repository-Ddq_GQbi.js";function j(e,t){let n=new Map(t.map(e=>[e.id,e]));return[...e.map(e=>{let t=n.get(e.id);return t?(n.delete(e.id),{...t,...e,history:{...t.history||{},...e.history||{}}}):e}),...n.values()]}var M=r((()=>{})),N,P,F=r((()=>{a(),c(),u(),M(),S(),N=[`https://www.googleapis.com/auth/calendar`,`https://www.googleapis.com/auth/calendar.events`,`https://www.googleapis.com/auth/drive.file`],P=class{static render(){return`
    ...
    `}static setup(e){let t=e.querySelector(`#calendarSection`),n=e.querySelector(`#googleClientIdInput`),r=e.querySelector(`#calendarSettingForm`),i=e.querySelector(`#todoCalendarSelect`),a=e.querySelector(`#doneCalendarSelect`),s=e.querySelector(`#loadCalendarListBtn`),c=e.querySelector(`#doneCalendarManualInput`),u=e.querySelector(`#googleLoginBtn`),f=e.querySelector(`#googleLoginStatus`),g=e.querySelector(`#googleDriveSyncToggle`),v=e.querySelector(`#googleSaveStatus`),y=e.querySelector(`#googleDriveLinkStatus`),b=!1;if(!l.supportsLocalStorage()){t&&(t.style.display=`none`);return}if(!n||!r||!i||!a||!c||!u||!f||!y||!g)return;A().then(e=>{n.value=e.clientId,e.todoCalendarId&&(i.innerHTML=`<option value="${e.todoCalendarId}">${e.todoCalendarId}</option>`),c.value=e.doneCalendarId}),g.checked=l.googleDriveSyncEnabled,c.value=l.calendarTargetId;let x=()=>{if(!f)return;let e=_();f.textContent=e?`ログイン済み`:`未ログイン`,u.textContent=e?`Googleからログアウト`:`Googleにログイン`},S=async()=>{if(!g.checked){y.style.display=`none`,y.innerHTML=``;return}if(y.style.display=`block`,!_()){y.textContent=`Google Drive 保存先リンクはログイン後に表示されます。`;return}try{let e=await m();if(!e){y.textContent=`Google Drive 保存先ファイルはまだ作成されていません。`;return}y.innerHTML=`Google Drive 保存先: <a href="${e}" target="_blank" rel="noopener noreferrer">バックアップファイルを開く</a>`}catch{y.textContent=`Google Drive 保存先リンクの取得に失敗しました。`}};x(),S(),u.addEventListener(`click`,async()=>{if(_()){h(),x(),await S();return}let e=n.value.trim();if(!e){alert(`先に OAuth 2.0 Client ID を入力してください。`);return}try{await d(N,!0);let t=await A();if(await C({clientId:e,todoCalendarId:t.todoCalendarId,doneCalendarId:c.value.trim()||t.doneCalendarId}),x(),g.checked){let e=await o();if(e!==null){let t=j(l.tasks,e.tasks);l.tasks=t,await p(t,{forceOverwrite:!0})}else await p(l.tasks)}await S()}catch(e){x(),await S();let t=e instanceof Error?e.message:`Googleログインに失敗しました。`;alert(t)}}),s?.addEventListener(`click`,async()=>{if(!n.value.trim()){alert(`OAuth 2.0 Client ID を入力してください。`);return}try{let e=await A();await C({clientId:n.value,todoCalendarId:e.todoCalendarId,doneCalendarId:e.doneCalendarId});let t=await T(),r=[`<option value="">選択してください</option>`].concat(t.map(e=>`<option value="${e.id}">${e.summary} (${e.id})</option>`)).join(``);i.innerHTML=r,a.innerHTML=r,i.value=e.todoCalendarId,a.value=e.doneCalendarId}catch(e){let t=e instanceof Error?e.message:`一覧取得に失敗しました。`;alert(t)}}),a.addEventListener(`change`,()=>{b=!0}),g.addEventListener(`change`,()=>{S()}),r.addEventListener(`submit`,async e=>{e.preventDefault();try{let e=c.value.trim()||(b?a.value.trim():``);await C({clientId:n.value,todoCalendarId:i.value,doneCalendarId:e}),c.value=e,l.googleDriveSyncEnabled=g.checked,v&&(v.style.display=`inline`,setTimeout(()=>{v.style.display=`none`},2500)),x(),await S()}catch{alert(`設定保存に失敗しました。`)}})}}})),I,L=r((()=>{e(),I=class e{static excludeGoogleTodoTasks(e){return e.filter(e=>e.sourceType!==`google-todo`)}static render(){return`
      <div class="data-box">
    ...
    `}static setup(e){let t=e.querySelector(`#overdueReferenceDateInput`),n=e.querySelector(`#overdueReferenceDateForm`),r=e.querySelector(`#displaySaveStatus`);!t||!n||(t.value=l.overdueReferenceDate,n.addEventListener(`submit`,e=>{e.preventDefault(),t.value&&(l.overdueReferenceDate=t.value,r&&(r.style.display=`inline`,setTimeout(()=>{r.style.display=`none`},2500)))}))}}})),B,V=r((()=>{a(),w(),D(),B=class e{static render(){return`
      ${e.renderSound()}
    ...
    `}static setup(t){e.setupSound(t),e.setupNotification(t)}static setupSound(e){let t=e.querySelector(`#notificationSoundSelect`),n=e.querySelector(`#playSoundTestBtn`);t&&(t.value=l.notificationSound,t.addEventListener(`change`,()=>{l.notificationSound=t.value})),n&&n.addEventListener(`click`,()=>{x.playSelected()})}static setupNotification(e){let t=e.querySelector(`#notificationSection`),n=e.querySelector(`#notificationEnableBtn`),r=e.querySelector(`#sendTestNotificationBtn`);if(!O.isSupported()){t&&(t.style.display=`none`);return}O.syncTestButtons(n,r),n&&n.addEventListener(`click`,async()=>{let e=await O.requestPermission();O.syncTestButtons(n,r),e===`granted`&&alert(`通知を有効にしました。`)}),r&&r.addEventListener(`click`,()=>{O.sendTestNotification()||alert(`先に通知を有効にしてください。`)})}}})),H,U=r((()=>{a(),H=class e{static render(){return`
    ...
    `}static applyTheme(e){let t=document.documentElement;if(e===`system`){t.removeAttribute(`data-theme`);return}t.setAttribute(`data-theme`,e)}static setup(t){let n=l.appTheme;e.applyTheme(n);let r=t.querySelector(`input[name="theme"][value="${n}"]`);r&&(r.checked=!0),t.querySelectorAll(`input[name="theme"]`).forEach(t=>{t.addEventListener(`change`,()=>{let n=t.value;l.appTheme=n,e.applyTheme(n)})})}}}));t((()=>{n(),y(),b(),E(),F(),L(),z(),V(),U(),f();var e=class extends HTMLElement{_taskRepository=new k;static get NAME(){return`done-settings`}connectedCallback(){this.render(),this.setup()}render(){this.innerHTML=`
      <main>
        ${P.render()}
        ${H.render()}
        ${R.render()}
    ...
    `}async setup(){g.startGoogleSessionKeepAlive(),this._taskRepository.hydrateFromLocal(),H.setup(this),P.setup(this),R.setup(this),B.setup(this),I.setup(this,this._taskRepository),document.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof Element))return;let n=t.closest(`a[href]`);n&&n.getAttribute(`href`)===`index.html`&&k.markNextIndexNavigationFromSettings()}),this._taskRepository.refreshFromCloudIfNeeded()}};customElements.get(e.NAME)||customElements.define(e.NAME,e),document.addEventListener(`DOMContentLoaded`,async()=>{let t=document.querySelector(`.container`);if(!t)return;let n=document.createElement(v.NAME);n.active=`settings`,t.appendChild(n);let r=document.createElement(e.NAME);t.appendChild(r);let i=document.createElement(s.NAME);t.appendChild(i)})}))();