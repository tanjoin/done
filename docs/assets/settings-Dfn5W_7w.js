import{D as e,E as t,M as n,N as r,O as i,P as a,T as o,_ as s,a as c,b as l,d as u,f as d,g as f,i as p,k as m,l as h,m as g,n as _,o as v,p as y,r as b,s as x,t as S,u as C,v as w,w as T,y as E}from"./google-auth-alert-OgUWiMkX.js";import{a as D,c as O,d as k,i as A,l as j,n as M,o as N,r as P,t as F,u as I}from"./task-repository-QRiOORuB.js";var L,R=a((()=>{o(),E(),v(),O(),d(),L=class{static render(){return`
    ...
    `}static setup(e,t){let n=e.querySelector(`#calendarSection`),r=e.querySelector(`#googleClientIdInput`),i=e.querySelector(`#calendarSettingForm`),a=e.querySelector(`#todoCalendarSelect`),o=e.querySelector(`#doneCalendarSelect`),s=e.querySelector(`#loadCalendarListBtn`),d=e.querySelector(`#doneCalendarManualInput`),p=e.querySelector(`#googleLoginBtn`),m=e.querySelector(`#googleLoginStatus`),h=e.querySelector(`#googleSaveStatus`),_=e.querySelector(`#googleDriveLinkStatus`),v=!1;if(!T.supportsLocalStorage()){n&&(n.style.display=`none`);return}if(!r||!i||!a||!o||!d||!p||!m||!_)return;I().then(e=>{r.value=e.clientId,e.todoCalendarId&&(a.innerHTML=`<option value="${e.todoCalendarId}">${e.todoCalendarId}</option>`),d.value=e.doneCalendarId}),d.value=T.calendarTargetId;let b=()=>{if(!m)return;let e=w();m.textContent=e?`ログイン済み`:`未ログイン`,p.textContent=e?`Googleからログアウト`:`Googleにログイン`},S=async()=>{if(_.style.display=`block`,!w()){_.textContent=`Google Drive 保存先リンクはログイン後に表示されます。`;return}try{let e=await c();if(!e){_.textContent=`Google Drive 保存先ファイルはまだ作成されていません。`;return}_.innerHTML=`Google Drive 保存先: <a href="${e}" target="_blank" rel="noopener noreferrer">バックアップファイルを開く</a>`}catch{_.textContent=`Google Drive 保存先リンクの取得に失敗しました。`}};b(),S(),p.addEventListener(`click`,async()=>{if(w()){g(),b(),u.notifyGoogleSessionStateChanged(),await S(),t.hide();return}let e=r.value.trim();if(!e){t.show(`先に OAuth 2.0 Client ID を入力してからログインしてください。`);return}try{let n=await I();await k({clientId:e,todoCalendarId:n.todoCalendarId,doneCalendarId:d.value.trim()||n.doneCalendarId}),await f(y,!0),b(),u.notifyGoogleSessionStateChanged();let r=await x();r!==null&&!T.taskSyncDirty&&(T.tasks=r.tasks,r.updatedAt&&(T.tasksLastUpdatedAt=r.updatedAt),T.taskSyncState={baseRevision:r.revision,baseDriveVersion:r.version,fileId:r.fileId,dirty:!1,baseTasks:r.tasks}),u.notifyGoogleLoginSucceeded(),await S(),t.hide()}catch(e){b(),u.notifyGoogleSessionStateChanged(),await S();let n=e instanceof Error?e.message:`Googleログインに失敗しました。`;t.show(n)}}),s?.addEventListener(`click`,async()=>{if(!r.value.trim()){t.show(`OAuth 2.0 Client ID を入力してからカレンダー一覧を取得してください。`);return}try{let e=await I();await k({clientId:r.value,todoCalendarId:e.todoCalendarId,doneCalendarId:e.doneCalendarId});let n=await j(),i=[`<option value="">選択してください</option>`].concat(n.map(e=>`<option value="${e.id}">${e.summary} (${e.id})</option>`)).join(``);a.innerHTML=i,o.innerHTML=i,a.value=e.todoCalendarId,o.value=e.doneCalendarId,t.hide()}catch(e){let n=l(e),r=e instanceof Error?e.message:`一覧取得に失敗しました。`;if(n){t.show(r);return}alert(r)}}),o.addEventListener(`change`,()=>{v=!0}),i.addEventListener(`submit`,async e=>{e.preventDefault();try{let e=d.value.trim()||(v?o.value.trim():``);await k({clientId:r.value,todoCalendarId:a.value,doneCalendarId:e}),d.value=e,h&&(h.style.display=`inline`,setTimeout(()=>{h.style.display=`none`},2500)),b(),await S()}catch{alert(`設定保存に失敗しました。`)}})}}})),z,B=a((()=>{m(),v(),z=class e{static excludeGoogleTodoTasks(e){return e.filter(e=>e.sourceType!==`google-todo`)}static render(){return`
      <div class="data-box">
