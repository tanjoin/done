import{D as e,E as t,M as n,N as r,O as i,P as a,T as o,_ as s,a as c,b as l,d as u,f as d,g as f,i as p,k as m,l as h,m as g,n as _,o as v,p as y,r as b,s as x,t as S,u as C,v as w,w as T,y as E}from"./google-auth-alert-AixUlIes.js";import{a as D,c as O,d as k,i as A,l as j,n as M,o as N,r as P,t as F,u as I}from"./task-repository-C84Ld-Lp.js";var L,R=a((()=>{o(),E(),v(),O(),d(),L=class{static render(){return`
    ...
    `}static setup(e,t){let n=e.querySelector(`#calendarSection`),r=e.querySelector(`#googleClientIdInput`),i=e.querySelector(`#calendarSettingForm`),a=e.querySelector(`#todoCalendarSelect`),o=e.querySelector(`#todoCalendarSelect2`),s=e.querySelector(`#doneCalendarSelect`),d=e.querySelector(`#skipSecondCalendarPeacockToggle`),p=e.querySelector(`#treatSecondCalendarAsLongTermToggle`),m=e.querySelector(`#loadCalendarListBtn`),h=e.querySelector(`#doneCalendarManualInput`),_=e.querySelector(`#googleLoginBtn`),v=e.querySelector(`#googleLoginStatus`),b=e.querySelector(`#googleSaveStatus`),S=e.querySelector(`#googleDriveLinkStatus`),C=!1;if(!T.supportsLocalStorage()){n&&(n.style.display=`none`);return}if(!r||!i||!a||!o||!s||!h||!_||!v||!S)return;I().then(e=>{r.value=e.clientId;let t=e.todoCalendarIds.length?e.todoCalendarIds:e.todoCalendarId?[e.todoCalendarId]:[],n=[`<option value="">選択してください</option>`];a.innerHTML=n.join(``),o.innerHTML=n.join(``),t[0]&&(a.innerHTML=`<option value="${t[0]}">${t[0]}</option>`),t[1]&&(o.innerHTML=`<option value="${t[1]}">${t[1]}</option>`),a.value=t[0]||``,o.value=t[1]||``,h.value=e.doneCalendarId}),h.value=T.calendarTargetId,d&&(d.checked=T.skipSecondCalendarPeacock,d.addEventListener(`change`,()=>{T.skipSecondCalendarPeacock=d.checked})),p&&(p.checked=T.treatSecondCalendarAsLongTerm,p.addEventListener(`change`,()=>{T.treatSecondCalendarAsLongTerm=p.checked}));let E=()=>{if(!v)return;let e=w();v.textContent=e?`ログイン済み`:`未ログイン`,_.textContent=e?`Googleからログアウト`:`Googleにログイン`},D=async()=>{if(S.style.display=`block`,!w()){S.textContent=`Google Drive 保存先リンクはログイン後に表示されます。`;return}try{let e=await c();if(!e){S.textContent=`Google Drive 保存先ファイルはまだ作成されていません。`;return}S.innerHTML=`Google Drive 保存先: <a href="${e}" target="_blank" rel="noopener noreferrer">バックアップファイルを開く</a>`}catch{S.textContent=`Google Drive 保存先リンクの取得に失敗しました。`}};E(),D(),_.addEventListener(`click`,async()=>{if(w()){g(),E(),u.notifyGoogleSessionStateChanged(),await D(),t.hide();return}let e=r.value.trim();if(!e){t.show(`先に OAuth 2.0 Client ID を入力してからログインしてください。`);return}try{let n=await I();await k({clientId:e,todoCalendarIds:n.todoCalendarIds,doneCalendarId:h.value.trim()||n.doneCalendarId}),await f(y,!0),E(),u.notifyGoogleSessionStateChanged();let r=await x();r!==null&&!T.taskSyncDirty&&(T.tasks=r.tasks,r.updatedAt&&(T.tasksLastUpdatedAt=r.updatedAt),T.taskSyncState={baseRevision:r.revision,baseDriveVersion:r.version,fileId:r.fileId,dirty:!1,baseTasks:r.tasks}),u.notifyGoogleLoginSucceeded(),await D(),t.hide()}catch(e){E(),u.notifyGoogleSessionStateChanged(),await D();let n=e instanceof Error?e.message:`Googleログインに失敗しました。`;t.show(n)}}),m?.addEventListener(`click`,async()=>{if(!r.value.trim()){t.show(`OAuth 2.0 Client ID を入力してからカレンダー一覧を取得してください。`);return}try{let e=await I();await k({clientId:r.value,todoCalendarIds:e.todoCalendarIds,doneCalendarId:e.doneCalendarId});let n=await j(),i=[`<option value="">選択してください</option>`].concat(n.map(e=>`<option value="${e.id}">${e.summary} (${e.id})</option>`)).join(``);a.innerHTML=i,o.innerHTML=i,s.innerHTML=i;let c=e.todoCalendarIds.length?e.todoCalendarIds:e.todoCalendarId?[e.todoCalendarId]:[];a.value=c[0]||``,o.value=c[1]||``,s.value=e.doneCalendarId,t.hide()}catch(e){let n=l(e),r=e instanceof Error?e.message:`一覧取得に失敗しました。`;if(n){t.show(r);return}alert(r)}}),s.addEventListener(`change`,()=>{C=!0}),i.addEventListener(`submit`,async e=>{e.preventDefault();try{let e=h.value.trim()||(C?s.value.trim():``),t=[a.value,o.value].map(e=>e.trim()).filter(Boolean);await k({clientId:r.value,todoCalendarIds:t,doneCalendarId:e}),h.value=e,b&&(b.style.display=`inline`,setTimeout(()=>{b.style.display=`none`},2500)),E(),await D()}catch{alert(`設定保存に失敗しました。`)}})}}})),z,B=a((()=>{m(),v(),z=class e{static excludeGoogleTodoTasks(e){return e.filter(e=>e.sourceType!==`google-todo`)}static render(){return`
      <div class="data-box">
...
ログイン中かつ Drive 同期ON の場合、Drive の最新データで上書きされます。`))try{await e.resetToDefault(),alert(`初期設定に戻しました。`)}catch{alert(`初期設定への復元に失敗しました。`)}}}})),V,H=a((()=>{o(),V=class{static render(){return`
