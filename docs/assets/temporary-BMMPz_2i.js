import{a as e,c as t,d as n,i as r,l as i,n as a,o,r as s,s as c,t as l,u}from"./local-storage-manager-DZ6mhuG2.js";n(),o(),r(),a(),u();var d=`done-temporary-form`,f=`done-temporary-form-reset`,p=`done-temporary-form-group-suggestions-update`,m=class extends HTMLElement{static get NAME(){return d}connectedCallback(){let e=this.renderForm();this.renderDate(e),this.renderGroup(e),this.renderText(e),this.renderTime(e),this.renderRemindMinutesBefore(e),this.renderDescription(e),this.renderLink(e),this.renderStrictMode(e),this.renderSubmitButton(e),this.setup()}setup(){this.setupTheme(),this.setupDefaultDate(),this.setupGroupSuggestions(),document.addEventListener(f,()=>{this.resetForm()}),document.addEventListener(p,()=>{this.setupGroupSuggestions()})}setupGroupSuggestions(){let e=document.getElementById(`groupSuggestions`);if(!e)return;e.innerHTML=``;let t=l.tasks;if(t)try{if(Array.isArray(t)){let n=new Set;t.forEach(e=>{e.group&&e.group.trim()!==``&&n.add(e.group.trim())}),n.forEach(t=>{let n=document.createElement(`option`);n.value=t,e.appendChild(n)})}}catch{}}resetForm(){document.getElementById(`group`).value=``,document.getElementById(`text`).value=``,document.getElementById(`endDate`).value=``,document.getElementById(`startTime`).value=``,document.getElementById(`endTime`).value=``,document.getElementById(`remindMinutesBefore`).value=``,document.getElementById(`description`).value=``,document.getElementById(`link`).value=``,document.getElementById(`strictMode`).checked=!1}setupDefaultDate(){document.getElementById(`date`).value=i.today,document.getElementById(`endDate`).value=``}setupTheme(){try{let e=l.appTheme;e&&e!==`system`?document.documentElement.setAttribute(`data-theme`,e):document.documentElement.removeAttribute(`data-theme`)}catch{}}renderDate(e){let t=document.createElement(`div`);t.style.width=`100%`,t.style.display=`flex`,t.style.gap=`16px`,t.style.flexWrap=`wrap`,t.innerHTML=`
    ...
    `}copyTemplateToForm(e,t){t.stopPropagation();let n=this._tempHistory.find(t=>t.id===e);n&&(this.handleCopyToFormEvent(n),window.scrollTo({top:0,behavior:`smooth`}))}deleteHistory(e,t){t.stopPropagation(),this._tempHistory=this._tempHistory.filter(t=>t.id!==e),l.temporaryInputHistory=this._tempHistory,this.renderHistory()}renderHistory(){let e=document.getElementById(`historyContainer`);if(e){if(e.innerHTML=``,this._tempHistory.length===0){e.innerHTML=`<p style="color: var(--text-muted); font-style: italic;">過去に追加したタスク履歴はありません。</p>`;return}this._tempHistory.forEach(t=>{let n=document.createElement(`div`);n.className=`history-item`;let r=t.startTime||t.endTime?` (${t.startTime||`00:00`}〜${t.endTime||`23:59`})`:``,i=t.remindMinutesBefore!==null&&t.remindMinutesBefore!==void 0?`<br><small>通知: ${t.remindMinutesBefore} 分前</small>`:``,a=t.group?`[${t.group}] `:``,o=t.endDate?`<br><small>終了日: ${t.endDate}</small>`:``;n.innerHTML=`
    ...
    `}static publishTemporaryTask(t){t.preventDefault();let n=e.getInputElement(`date`).value,r=e.getInputElement(`endDate`).value,i=e.getInputElement(`group`).value.trim(),a=e.getInputElement(`text`).value.trim(),o=e.getInputElement(`startTime`).value,s=e.getInputElement(`endTime`).value,u=e.getInputElement(`remindMinutesBefore`).value,d=e.getInputElement(`description`).value.trim(),m=e.getInputElement(`link`).value.trim(),h=e.getInputElement(`strictMode`).checked,v=null;if(u!==``){let e=Number(u);if(!Number.isFinite(e)||e<0){alert(`通知猶予は 0 以上の数値で入力してください。`);return}v=Math.floor(e)}let y=l.tasks;if(r&&r<n){alert(`終了日は開始日と同じかそれ以降の日付にしてください。`);return}let b=new c({id:`actual_temp_`+Date.now()+`_`+Math.random().toString(36).substring(2,7),group:i,text:a,startTime:o,endTime:s,remindMinutesBefore:v,description:d,link:m,strictMode:h,history:{},notifiedDate:``,specificDate:n,endDate:r});y.push(b),l.tasks=y;let x={id:`hist_`+Date.now(),group:i,text:a,endDate:r,startTime:o,endTime:s,remindMinutesBefore:v,description:d,link:m,strictMode:h};document.dispatchEvent(new CustomEvent(g,{detail:x,bubbles:!0})),document.dispatchEvent(new CustomEvent(f,{bubbles:!0})),document.dispatchEvent(new CustomEvent(_,{bubbles:!0})),document.dispatchEvent(new CustomEvent(p,{bubbles:!0}))}static getInputElement(e){return document.getElementById(e)}};customElements.get(y.NAME)||customElements.define(y.NAME,y),document.addEventListener(`DOMContentLoaded`,async()=>{let t=document.querySelector(`.container`);if(t){let n=document.createElement(s.NAME);n.active=`temporary`,t.appendChild(n);let r=document.createElement(`h3`);r.className=`group-title`,r.textContent=`一時的タスクを追加する`,t.appendChild(r);let i=document.createElement(y.NAME);t.appendChild(i),document.getElementById(`taskForm`).addEventListener(`submit`,y.publishTemporaryTask);let a=document.createElement(e.NAME);t.appendChild(a)}});