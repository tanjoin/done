...
export default class IndexFilterControls extends HTMLElement {
  ...
  private setupHideNonTargetDayBtn(): void {
    const hideNonTargetDayBtn = this.querySelector('#hideNonTargetDayBtn') as HTMLButtonElement | null;
    if (!hideNonTargetDayBtn) return;
    this.setFilterButtonState(hideNonTargetDayBtn, LocalStorageManager.filterHideNonTargetDay);
    hideNonTargetDayBtn.addEventListener('click', () => this.toggleFilterState(hideNonTargetDayBtn, LocalStorageManager.FILTER_HIDE_NON_TARGET_DAY_KEY));
  }

  private setupHideOutOfTimeBtn(): void {
    const hideOutOfTimeBtn = this.querySelector('#hideOutOfTimeBtn') as HTMLButtonElement | null;
    if (!hideOutOfTimeBtn) return;
    this.setFilterButtonState(hideOutOfTimeBtn, LocalStorageManager.filterHideOutOfTime);
    hideOutOfTimeBtn.addEventListener('click', () => this.toggleFilterState(hideOutOfTimeBtn, LocalStorageManager.FILTER_HIDE_OUT_OF_TIME_KEY));
  }

  private setupHideCompletedBtn(): void {
    const hideCompletedBtn = this.querySelector('#hideCompletedBtn') as HTMLButtonElement | null;
    if (!hideCompletedBtn) return;
    this.setFilterButtonState(hideCompletedBtn, LocalStorageManager.filterHideCompleted);
    hideCompletedBtn.addEventListener('click', () => this.toggleFilterState(hideCompletedBtn, LocalStorageManager.FILTER_HIDE_COMPLETED_KEY));
  }

  private setupHideCancelledBtn(): void {
    const hideCancelledBtn = this.querySelector('#hideCancelledBtn') as HTMLButtonElement | null;
    if (!hideCancelledBtn) return;
    this.setFilterButtonState(hideCancelledBtn, LocalStorageManager.filterHideCancelled);
    hideCancelledBtn.addEventListener('click', () => this.toggleFilterState(hideCancelledBtn, LocalStorageManager.FILTER_HIDE_CANCELLED_KEY));
  }

  private setFilterButtonState(button: HTMLButtonElement, isHidden: boolean): void {
    if (!button) return;
  ...
  private toggleFilterState(button: HTMLButtonElement, key: string): void {
    ...
    this.dispatchEvent(new CustomEvent(IndexFilterControls.EVENT_FILTER_CHANGE, {
