...
export default class NotificationSound {
  ...
  static playOriginal() {
    try {
      ...
      const chimeNotes = [
        ...
        {beat: 1, note: 60},
        {beat: 2, note: 62},
        {beat: 3, note: 55}, // キンコンカンコン
        {beat: 5, note: 55},
        {beat: 6, note: 62},
        {beat: 7, note: 64},
        {beat: 8, note: 60}, // コンカンキンコン
      ];
    ...
    try {
      ...
      notes.forEach((note, i) => {
        ...
        partials.forEach(partial => {
          ...
          gain.gain.exponentialRampToValueAtTime(
            0.00001,
            noteTime + duration * (0.8 / partial.ratio),
          );
  ...
  static playLinePokipoki() {
    try {
      ...
      const clicks = [
        {time: 0.0, startFreq: 1400, endFreq: 400, dur: 0.035, vol: 0.25},
  ...
  static playJojo() {
    try {
      ...
      const seq = [
        {note: 79, delay: 0.0, duration: 1.0},
        {note: 83, delay: 0.08, duration: 1.2},
        {note: 88, delay: 0.16, duration: 1.5},
      ];
      ...
      seq.forEach(item => {
        ...
        partials.forEach(partial => {
          ...
          osc.frequency.setValueAtTime(
            noteToFreq(item.note) * partial.ratio,
            t,
          );
